...
class intellectual extends StatefulWidget {
  const intellectual({Key? key}) : super(key: key);
  @override
...
class _intellectualState extends State<intellectual> {
  ...
  FilterOption selectedFilter = FilterOption
      .Immigration; // Set the initial filter

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("IntellectualProperty Lawyers"),
      ),
      body: Column(
        children: [
          Expanded(
            child: StreamBuilder(
              stream: selectedFilter == FilterOption.All
                  ? users.snapshots()
                  : users
                  .where('filter', isEqualTo: 'IntellectualProperty')
                  .snapshots(),
